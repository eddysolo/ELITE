...
for jj = 1: length(caseInx)
    ...
    for sl = sliceInx(jj)
        ...
        if breastData
            % reading tissue mask
            load ([pathName '/' 'BC' num2str(caseInx(jj),'%02.f') '_mask_sl' num2str(sl) '.mat'])
        end

        if flags.highRes
            disp('reading recon by ResNet')
            ...
            file = ['/GRASP_pred_lowRes_BC' num2str(caseInx(jj),'%02.f') '_slice' num2str(sl) '.mat'];
    ...
    save([pathName,'GRASP_Pro_kArr_roi_BC23_sl69_highRes.mat'],'GRASP_Pro_kArr');
    s = toc(tt);
    h = fix(s/3600);
    s = mod(s,3600);
    m = fix(s/60);
