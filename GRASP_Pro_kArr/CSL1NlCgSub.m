function [x,FF] = CSL1NlCgSub(x0,param)
% 
% res = CSL1NlCgSub(param)
...
if param.TVWeight
    w = param.TV*(param.PCA'*(x+t*dx)); 
    %w = param.SP*(param.PCA'*(x+t*dx)); 
...
if param.TVWeight
    ...
    w = param.TV*(param.PCA'*x);
    TVGrad = param.TV'*(w.*(w.*conj(w)+param.l1Smooth).^(-0.5));
    %w = param.SP*(param.PCA'*x);
    %TVGrad = param.SP'*(w.*(w.*conj(w)+param.l1Smooth).^(-0.5));
    TVGrad=param.PCA*TVGrad;
    
    
    %TVGrad = TV_image(param.PCA'*x);
